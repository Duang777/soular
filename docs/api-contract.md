...
GitHub Pages 不读取跨站 Cookie；未登录用户可以浏览首页，点击内容后弹出前往正式站的
登录引导。开发模式跳过门槛以便本地调试。
...
React 外壳根据该接口决定是否挂载受保护内容：未登录时保留首页，点击内容后展示登录引导；
查询失败时提供重试。授权后首页展示已连接账号。`profile` 获取失败时仍保留已授权状态，
不伪造昵称或头像。知乎 `/user` 没有稳定响应 schema，服务端兼容嵌套或
...
该接口，只有账号版本一致时才接收结果；标签重新可见时会重新校验登录账号。OAuth 成功
回跳后首页自动打开“知乎兴趣星谱”校准台，账号区域可再次打开。接口返回前只显示不确定
加载状态，不生成模拟进度或推测关键词；成功后展示四项汇总数量、最多六个兴趣词及其
相对强度，失败时允许重试且不阻断其他体验。
...
- 服务端个性化推荐：不开放独立 API；当前推荐只在已审核快照中使用本地表态和裁剪后的
  兴趣信号计算。
...
模型每条回答只返回 `i`（原始序号）、`s`（立场，-1..1）、`r`（离轴度，0..1）和 `p`（九派）
四个字段，不再返回观点正文；`claim` 统一由回答摘要截取前 72 字生成。缩短输出是为了让
单次调用覆盖全部回答，直答有独立速率限制且不支持并发，不得改成按批多次调用。
`r` 表示该回答有多大程度是在回答当前立场轴，用于把「真折中」与「在谈别的事」区分开；
未被模型标注的回答记为 `relevance: 0`，不得与立场居中的回答混为一谈。
`axis` 的三段立场各由两个字段组成：`left`/`center`/`right` 是主张，不超过 12 字，说清
这一端认为什么；`leftReason`/`centerReason`/`rightReason` 是理由，不超过 20 字，说清凭
什么这样认为，超长部分按字截断并补省略号。两者都必须是加上「我觉得」能读通的大白话，
但「我觉得」只是自检标准，不得出现在字段值里：实测模型会把这三个字当成前缀写进输出，
占掉 12 字预算中的 3 字并导致主张被截断，prompt 必须显式禁止输出该字样。
既不允许 支持/反对/质疑/中立/乐观/悲观 这类态度词，也不允许 共同体/生态/可验证/形式化/
主体性/范式 这类圈内词；主张必须是能被反驳的断言，不能是名词短语。
`left` 与 `right` 必须互相反对，认同一端的人一定不认同另一端；两端若只是同一立场的
两种说法（例如都在指责同一方，只是一个讲法律一个讲道德），该轴无效，必须换轴。
中点必须是两端之间的真实折中。模型未返回主张时回退到通用文案；未返回理由时留空串，
前端按单行极标降级，不得编造理由。

「我觉得」等表态前缀由解析层在 `clip` 之前强制剥离，不依赖模型遵守 prompt。

模型原文经可选回调透出，由 `prepare-question-preset` 写入 staging 产物的 `rawResponse`
字段，供人工核对 `clip` 截断前的措辞。该字段不属于 `QuestionSpectrum`，不会随快照发布，
也不出现在任何 HTTP 响应中。

### 作者补全与快照组装

`question_answers` 分页只返回内容类型、标识、链接和摘要，没有作者名、头像和赞同数。
这三项只能经站内搜索 `/api/v1/content/zhihu_search` 获取，由 `ZhihuClient.zhihuSearch`
封装，与只搜全网、明确拒绝知乎域名的 `globalSearch` 不是同一个接口。
实测上游恒定返回 10 条，`Count` 填 10、20、50 结果完全一致，更换 query 写法命中数也一致，
因此该方法不开放 `Count` 参数。搜索结果与分页结果按 answer id 联结，`ContentToken` 与
链接中的 answer id 相等，联结可靠；但每题通常只有 2-3 条能对上，其余回答无法补全作者。

因此组装出的快照是前景背景分层的：能补全的用真实作者名、赞同数和 zhimg.com 头像，
其余用中性编号「知乎答主 NN」与仓库自有的九派素材，不得编造身份或赞同数。
人物行第 8 位是头像地址，只允许 https 的 zhimg.com 远程图或本地素材路径；缺省时沿用
`avatarBase` 的逐条 jpg 约定。第 9 位是离轴度，前端据此把偏题回答画暗但不隐藏。
渲染时必须区分「未标注」与「偏题」：`relevance` 为 0 表示模型没有标注这一条，不得据此
降低亮度；只有大于 0 且低于阈值才按偏题处理。行长不足 9 位的既有快照一律按未标注处理。

`leftChoice`、`rightChoice`、`leftTendency`、`rightTendency` 是人工快照才填的短标签，
自动组装的快照不产出；渲染前必须统一回退到对应的 `left`/`right` 主张，不得把 `undefined`
透传到筛选条或卡片。搜索补全结果写回 staging，重跑组装不再消耗配额。
...
身份修订号供 iframe 在切号时清理旧头像、碰撞、圈子和聚焦状态。未登录状态和 GitHub
Pages 镜像只挂载首页展示所需的角色波浪，不挂载星云和人格内容 iframe；已登录但资料读取
失败时，以及开发模式中，继续使用本地“我”占位头像和无画像匹配。

首次登录首页导览完成后，React 外壳可在用户真正进入探索态时发送一次无载荷的
`nebula-open-guide` 消息，复用星云已有向导；确认页和用户选择跳过时不发送。

画像关键词不决定用户在单个问题中的立场或人格派别。人格仍由该星云中的有效点赞计算；
关键词只显示为兴趣底色，并以 18% 的次级权重参与同频、互补候选排序，本题立场权重为
82%。兴趣相关性优先匹配完整关键词，其次使用英文 token 或中文双字片段，不使用单字
重叠推断相关性。画像信号只随当前标签页的临时人格上下文传到卡片页，不写入公开分享 URL；该上下文
绑定账号版本，卡片页在挂载、重新获得焦点或重新可见时复查授权状态，版本不一致便清理。

观点星云内的搜索只查询本地快照目录、回答者和观点摘要；推荐在本地按“本题立场 82%、
知乎兴趣 18%”生成稳定排名。同频和互补模式各展示前三位候选及两项匹配依据，相关推荐
复用同频排序；尚未点赞时仍返回光谱两端与中点的代表观点。热榜使用上述只读接口并
