...
- 知乎 `openapi.zhihu.com/user` 基础资料接口只使用 OAuth Access Token 作为 Bearer，不携带 Access Secret、`X-OAuth-Token` 或时间戳；双凭证仅用于 `developer.zhihu.com/api/v1/user/...` 用户数据接口。公开资料解析需兼容已观测的顶层/嵌套对象与头像字段。
- OAuth 状态接口只读取 Session，不同步补取资料；已有 Session 的资料补取走独立接口，按 Token 指纹缓存并保留旧资料中的非空字段。缓存版本升级时可按原始年龄兼容读取仍有效的正缓存，但不得写回续期，旧负缓存直接失效。状态、资料和画像响应使用同一账号版本供前端拒绝跨账号组合。
