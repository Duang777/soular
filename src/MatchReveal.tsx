...
export function MatchRevealPage() {
  ...
  const nebulaTarget = payload.version !== currentVersion
    ? `/nebula?preset=${encodeURIComponent(payload.preset)}&version=${encodeURIComponent(payload.version)}`
    : `/nebula?preset=${encodeURIComponent(payload.preset)}`;
  ...
  return (
    <div className="match-page">
      <nav className="shelf-nav shelf-nav--nebula match-nav" aria-label="对照导航">
        <div className="shelf-nav__tags">
          <Link to="/" className="shelf-tag">返回首页</Link>
        </div>
        <p className="shelf-nav__cast">观点对照</p>
      </nav>
