...
import { nebulaPresetVersion, resolveNebulaPreset } from "./people";
...
export function parseShareMatchQuery(params: URLSearchParams): ShareMatchPayload | null {
  const preset = resolveNebulaPreset(params.get("preset") ?? "");
  const version = params.get("version") ?? "";
  const cast = params.get("cast") ?? "";
  const stanceCode = Number(params.get("s"));

  if (!nebulaPresetVersion(preset)) return null;
