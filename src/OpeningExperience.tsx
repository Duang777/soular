...
function drawFrame(
  ...
  const zhihuBlend = easeInOut(
    cue(
      OPENING_TIMELINE_MS.zhihuGatherStart,
      OPENING_TIMELINE_MS.zhihuGatherEnd,
    ),
  );
  const soularBlend = easeInOut(
    cue(OPENING_TIMELINE_MS.soularStart, OPENING_TIMELINE_MS.soular),
  );
  const partnershipBlend = easeInOut(
    cue(OPENING_TIMELINE_MS.partnershipStart, OPENING_TIMELINE_MS.partnership),
  );
  const galaxyCue = cue(
    OPENING_TIMELINE_MS.reveal,
    OPENING_TIMELINE_MS.galaxySettle,
  );
...
export function OpeningExperience({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const phaseTimers = [
      window.setTimeout(() => setPhase("soular"), OPENING_TIMELINE_MS.soular),
      window.setTimeout(() => setPhase("partnership"), OPENING_TIMELINE_MS.partnership),
      window.setTimeout(() => setPhase("reveal"), OPENING_TIMELINE_MS.reveal),
